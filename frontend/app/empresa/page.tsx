...
export default function EmpresaPage() {
  ...
  useEffect(() => {
    void fetchCompanySummary().then((summary) => {
      ...
      setSummaryMetrics(Array.isArray(payload.metrics) ? payload.metrics : []);
      setSummaryAlerts(Array.isArray(payload.alerts) ? payload.alerts : []);
      setSummaryActivity(Array.isArray(payload.recentActivity) ? payload.recentActivity : []);
      setSummaryRadar(Array.isArray(payload.radar) ? payload.radar : []);
      setSummaryActions(Array.isArray(payload.recommendedActions) ? payload.recommendedActions : []);
      setSummaryQuestions(Array.isArray(payload.ai?.recommendedQuestions) ? payload.ai.recommendedQuestions : []);
    });
  }, []);

  const [summaryMetrics, setSummaryMetrics] = useState<ExecutiveMetric[]>([]);
  const [summaryAlerts, setSummaryAlerts] = useState<AlertItem[]>([]);
  const [summaryActivity, setSummaryActivity] = useState<ActivityItem[]>([]);
  const [summaryRadar, setSummaryRadar] = useState<RadarItem[]>([]);
  const [summaryActions, setSummaryActions] = useState<StrategicAction[]>([]);
          ...
          <section className="overflow-hidden rounded-3xl border border-cyan-100/15 bg-[linear-gradient(125deg,rgba(7,29,50,0.96),rgba(9,44,73,0.9),rgba(6,23,43,0.95))] p-5 shadow-xl shadow-slate-950/35 md:p-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {summaryMetrics.length ? (
                ...
                <EmptyData className="sm:col-span-2 xl:col-span-4" />
              )}
            </div>
          </section>
                ...
                {isAiThinking ? (
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="rounded-2xl border border-cyan-100/12 bg-slate-950/45 p-4">
                      ...
                      <ul className="mt-2 space-y-2">
                        {aiInsight.dataPoints.length ? (
                          ...
                          <li className="rounded-xl border border-dashed border-cyan-100/14 bg-white/5 px-3 py-2 text-xs text-cyan-100/70">
                            No hay datos para mostrar
                          </li>
                        )}
                      </ul>
                    ...
                    <div className="rounded-2xl border border-emerald-200/20 bg-emerald-300/10 p-4">
                      ...
                      <p className="mt-2 text-sm leading-7 text-emerald-50/90">{aiInsight.advice || "No hay datos para mostrar"}</p>
                      <p className="mt-3 text-xs text-emerald-100/85">Siguiente paso: {aiInsight.nextStep || "No hay datos para mostrar"}</p>
                    </div>
                  </div>
                ) : (
          ...
          <section className="tech-card">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {summaryActions.length ? (
                ...
                <EmptyData className="md:col-span-3" />
              )}
            </div>
          </section>
        </section>
